...
抽屉**不是**"把所有容器的每格上限调高"，而是一个专用方块：一格只装一种物品，但容量极大，
正面直接显示装的是什么、有多少，右键存取不用开界面。定位是"大宗囤积"（圆石、泥土、矿），
和箱子的"杂物"分工不同。

**容量分档**（单位是"组"，1 组 = 该物品的原版堆叠上限；圆石 40/组）：

| 档 | 容量 | 折合圆石 | 材料 |
|---|---|---|---|
| 木抽屉 | 64 组 | 2,560 | 木板 |
| 铜抽屉 | 256 组 | 10,240 | + 铜锭 |
| 铁抽屉 | 1,024 组 | 40,960 | + 铁锭 |
| 钻石抽屉 | 4,096 组 | 163,840 | + 钻石 |

**为什么不设无穷**：
1. 数量要显示、要同步、要存档，无穷让"数量"这个概念失去意义（UI 得做 `12.5k` 这类缩写）。
2. 无上限 = 一个抽屉吞掉全服产出，分类墙、物流、扩建这些玩法直接消失。
3. 槽位计数是 `int`，21 亿封顶，接近上限时加法会溢出。
4. 破坏抽屉要把内容变成可捡起物品，16 万个已经要分批生成掉落物了，再多就是灾难。

钻石抽屉 16 万个圆石，一面 3×3 的抽屉墙就是 150 万，支撑长期多人服务器绰绰有余。

**耐久物品怎么算**（已核实原版行为，不需要我们做任何事）：
`BlocksData.txt` 的 `MaxStacking` 列里，工具、衣服、箱子全都是 **1**，普通方块 40，钻石 4。
而且耐久值存在 data 位里，用过的镐和新镐是**不同的 value**，本来就不会合并。
所以规则很简单：**抽屉只接受 `MaxStacking > 1` 的物品**，工具衣服放不进去（放进去也只有 1 个，没意义）。

> 你提到的"满耐久可以叠加、用过就不行"——SC 原版满耐久工具也是 `MaxStacking = 1`，
> 要实现得覆写原版的堆叠规则，会改变原版平衡、也容易和别的 Mod 打架。**默认不做**，
> 如果确实想要，作为一个可关的设置项再议。

- 正面显示物品图标 + 数量：**当前实现为准星预览**（指着抽屉时屏幕提示"物品 × 数量/容量"）。
  真正画在方块正面需要自建一套世界空间渲染（原版告示牌那套 550 行），排在后面做。
- 右键放入 / 拿取一组；连上枢纽即进入网络。
- 升级：容量升级、抽取锁定（拿空后不忘记类型）、虚空（超出上限直接销毁）。

**附：堆叠升级（你说的"提高每格上限"）**——这是另一件事，属于容器升级件：
让**某个背包/箱子内部**的格子上限翻倍（×2 / ×4）。实现上只要我们自己的容器覆写 `GetSlotCapacity` 即可，
不影响玩家背包和原版容器。列入 P3 的第二批升级。
